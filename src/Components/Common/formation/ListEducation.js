...
const Listeducation = (props) => {
    ...
    const [updateForm, setUpdateForm] = useState(props.education);
    ...
    const UpdateSubmitHandlar =  async(event)=>{
        ...
        await axios.put('https://localhost:7154/api/education/edit', updateForm).then( res => {
    ...
    return (
        ...
        <div className='   m-2 pt-3 '> 
           ...
           <div className='  pt-3 m-2 '> 
                <button  onClick={()=>props.OnDelete(props.education.id)} className=' rounded-xl bg-red-500  p-2 hover:bg-red-400 shadow-md text-center text-white'><DeleteIcon  fontSize='small'color='inherit'/>
                </button>
                </div>
          
        </div>
        <Modeleducation education={props.education} open={open}  UpdateSubmitHandlar={UpdateSubmitHandlar}  onChangeHandler={onChangeHandler} handleClose={handleClose}/>
      </Card>
    );
}
