...
const ListProject=(props)=> {
    ...
    const [updateForm, setUpdateForm] = useState(props.project);
    ...
    const UpdateSubmitHandlar =  async(event)=>{
        ...
        await axios.put('https://localhost:7154/api/project/edit', updateForm).then( res => {
  ...
  return (
    ...
    <div className='   m-2 pt-3 '> 
       ...
       <div className='  pt-3 m-2 '> 
            <button  onClick={()=>props.OnDelete(props.project.id)} className=' rounded-xl bg-red-500  p-2 hover:bg-red-400 shadow-md text-center text-white'><DeleteIcon  fontSize='small'color='inherit'/>
            </button>
            </div>
      
    </div>
    <ModelProject setUpdateForm={setUpdateForm} updateForm={updateForm} project={props.project} open={open}  UpdateSubmitHandlar={UpdateSubmitHandlar}  onChangeHandler={onChangeHandler} handleClose={handleClose}/>
  </Card>
  )
}
